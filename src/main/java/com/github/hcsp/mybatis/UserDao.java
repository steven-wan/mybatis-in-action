...
public class UserDao {
    ...
    public Pagination<User> getUserByPage(String username, int pageSize, int pageNum) {
        ...
        try (SqlSession sqlSession = sqlSessionFactory.openSession()) {
            ...
            userList = sqlSession.selectList("userMapper.selectUser", username, rowBounds);
            totalUserCount = sqlSession.selectList("userMapper.selectUser", username).size();
        }
        totalUserCount = (totalUserCount % pageSize == 0 ? totalUserCount / pageSize : (totalUserCount / pageSize + 1));
        return Pagination.pageOf(userList, pageSize, pageNum, totalUserCount);
    }
    ...
    public void batchInsertUsers(List<User> users) {
        try (SqlSession sqlSession = sqlSessionFactory.openSession()) {
            sqlSession.insert("userMapper.batchInsertUsers", users);
            sqlSession.commit();
        }
    }
    ...
    public void updateUser(User user) {
        Objects.requireNonNull(user.getId(), "要修改的用户信息，其id必须不为null");
        try (SqlSession sqlSession = sqlSessionFactory.openSession()) {
